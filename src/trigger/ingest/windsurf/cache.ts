...
import type { CachedWindsurfRelease } from './types'
...
export function getCacheKey(toolSlug: string): string {
	...
	return ['windsurf', namespace, 'latest-release', toolSlug].join(':')
}
...
export async function readCachedRelease(
	...
	const cached = await redis.get<CachedWindsurfRelease>(getCacheKey(toolSlug))
	return cached ?? null
}

export async function writeCachedRelease(
	toolSlug: string,
	release: CachedWindsurfRelease,
): Promise<void> {
	const redis = getRedisClient()
	if (!redis) {
		return
