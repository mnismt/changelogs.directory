...
export const ingestGeminiCli = task({
	...
	run: async (
		...
		const toolSlug = payload.toolSlug || 'gemini-cli'
		const forceFullRescan = payload.forceFullRescan ?? false
		const startTime = Date.now()

		logger.info('Starting Gemini CLI changelog ingestion', { toolSlug })

		try {
			// ============================================================================
			// Phase 1: Setup
			// ============================================================================
			const setupResult = await setupStep(
				prisma,
				toolSlug,
				startTime,
				forceFullRescan,
...
export const ingestGeminiCliSchedule = schedules.task({
	id: 'ingest-gemini-cli-schedule',
	cron: '0 */6 * * *',
	run: async () => {
		await ingestGeminiCli.trigger({})
	},
})
