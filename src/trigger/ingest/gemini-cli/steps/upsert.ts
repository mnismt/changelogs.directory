...
export async function upsertStep(
	...
	for (const parsedRelease of enrichResult.enrichedReleases) {
		...
		if (existingRelease) {
			...
			if (isUnchanged && !ctx.forceFullRescan) {
				// No changes, skip
			...
			if (isUnchanged && ctx.forceFullRescan) {
				logger.info('Force full rescan enabled, reprocessing release', {
					version: parsedRelease.version,
				})
			} else {
			...
			await ctx.prisma.release.update({
				...
				data: {
					...
					sourceUrl:
						parsedRelease.sourceUrl ??
						`${ctx.tool.repositoryUrl}/blob/main/CHANGELOG.md#${parsedRelease.version.replace(/\./g, '')}`,
