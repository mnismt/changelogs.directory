...
export const ingestAntigravity = task({
	...
	run: async (
		...
		const toolSlug = payload.toolSlug || 'antigravity'
		const forceFullRescan = payload.forceFullRescan ?? false
		const startTime = Date.now()
		let ctx: IngestionContext | null = null

		logger.info('Starting Antigravity changelog ingestion', { toolSlug })
		...
		try {
			...
			if (ctx) {
				...
				logger.error('Antigravity ingestion failed before setup completed', {
					error,
				})
			}
...
export const ingestAntigravitySchedule = schedules.task({
	...
	cron: '0 */6 * * *',
	run: async () => {
		await ingestAntigravity.trigger({})
	},
})
