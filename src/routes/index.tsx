...
import { FeedSkeleton, HomePageSkeleton } from '@/components/home/home-skeleton'
...
export const Route = createFileRoute('/')({
	...
	loader: async () => {
		// Blocking: hero release + platform version (above the fold)
		const [heroData, platformChangelog] = await Promise.all([
			getLatestReleasesAcrossTools({
		...
		// Deferred: grouped releases feed (below the fold, streamed)
		const groupedDataPromise = getReleasesGroupedByTool({
		...
		return {
			...
			groupedData: defer(groupedDataPromise),
		}
	},
...
function HomePage() {
	const { heroData, groupedData, platformVersion } = Route.useLoaderData()
	...
	return (
		<div className="flex-1 pt-20 sm:pt-24 lg:pt-28">
					...
					<span className="text-foreground">
						{animationStep === 'prompt' ? (
							<span
								...
								style={{
									animation: 'typewriter-prompt 0.65s steps(13) forwards',
								}}
							>
								view releases
							</span>
			...
			{/* Feed Section - Streamed via Suspense */}
			<div
				className={cn(
			...
			>
				<div className="overflow-hidden min-h-0">
					<Suspense fallback={<FeedSkeleton />}>
						<Await promise={groupedData}>
							{(resolvedGroupedData) => (
								<HomeFeedSection initialGroupedData={resolvedGroupedData} />
							)}
						</Await>
					</Suspense>
				</div>
...
function HomeFeedSection({
	...
	initialGroupedData: Awaited<ReturnType<typeof getReleasesGroupedByTool>>
}) {
						...
						<div className="flex flex-col gap-3 bg-card/60 p-4 sm:flex-row sm:items-center sm:justify-between">
							...
							<div className="relative flex-1 sm:max-w-xs">
								<input
									...
									className="w-full rounded border border-border bg-secondary px-3 py-1.5 pr-8 font-mono text-xs text-foreground placeholder:text-muted-foreground/70 focus:border-foreground/30 focus:outline-none focus:ring-1 focus:ring-foreground/20 focus-visible:border-foreground/40 focus-visible:ring-2"
								/>
								{isSearching && (
